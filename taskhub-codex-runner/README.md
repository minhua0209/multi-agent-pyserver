...
```bash
./start_runner.sh http://192.168.170.18:8000 root --once
```

第二个参数必须是 TaskHub 后端用户的 `id`，例如 `root` 或 `user_xxx`，不能填写用户展示名。Runner 启动时会调用 `/api/v1/users/current` 校验身份，返回用户 ID 与配置不一致时直接停止。

常驻运行：

```bash
./start_runner.sh http://192.168.170.18:8000 root
```

启动 Web 控制台：

```bash
./start_runner.sh http://192.168.170.18:8000 root --ui
```
...
```bash
./start_runner.sh http://192.168.170.18:8000 root --ui --background
```
...
```bash
./start_runner.sh http://192.168.170.18:8000 root --install-skill
```

覆盖更新已安装 Skill：

```bash
./start_runner.sh http://192.168.170.18:8000 root --install-skill --update-skill
```
...
Runner 默认仅对自己创建的 Codex 子进程启用最高权限和免审批模式：

```text
codex exec --dangerously-bypass-approvals-and-sandbox --ephemeral
```

该配置不会修改 `~/.codex/config.toml`，也不会影响其他终端或项目中直接启动的 Codex。人工节点内容会进入一个可访问本机网络和文件系统的进程，因此只应连接可信 TaskHub；需要降权时可通过 `TASKHUB_CODEX_COMMAND` 显式覆盖。
...
```json
{
  ...
  "user_id": "root",
  "runner_id": "local-codex-runner",
  "ipc_dir": "/当前项目/taskhub-codex-runner/runtime/ipc"
}
```

其他 Codex 会话访问任务中心时，应优先读取 Skill runtime 里的 `runner_cli_path`，通过 runner CLI 间接访问 TaskHub。CLI 不直接发起 HTTP 请求，而是把命令写入 `runtime/ipc/requests`；宿主机常驻 Runner 读取命令、请求 TaskHub，再把结果写回 `runtime/ipc/responses`。因此 Skill 不需要知道 TaskHub 地址，Codex 沙箱也不需要网络权限。

使用 Runner CLI 前必须先启动常驻 Runner，推荐：

```bash
./start_runner.sh http://192.168.170.18:8000 root --ui --background
```

如果 Runner 未启动，CLI 会返回 `TaskHub runner command broker did not respond`，不会绕过 Runner 直接请求 TaskHub。
...
Codex Skill 默认通过 runner CLI 的本地文件 IPC 调用任务中心能力。网络请求只发生在宿主机 Runner 进程中，从而避免 Codex 沙箱的网络限制。
...
该简化确认命令会先读取任务详情，并根据意图识别草稿自动补齐完整 `contract`，不会再生成 `legacy_inferred` 兼容契约。

需要编辑任务目标、交付物目标、文件交付信息、验收标准或人工验收开关时，使用完整确认 JSON：
