...
Build and run the API service with Docker Compose:
...
On macOS Docker, containers must use `host.docker.internal` to connect to a
MySQL instance running on the host machine:

```text
mysql+pymysql://root:demo_root_123@host.docker.internal:3306/demo_db?charset=utf8mb4
```
